...
pub async fn fetch_latest_release(owner: &str, repo: &str) -> Result<GitHubRelease> {
    ...
    if !response.status().is_success() {
        anyhow::bail!("Failed to fetch release: HTTP {}", response.status().as_u16());
    }

    response.json().await.with_context(|| "Failed to parse release JSON")
}
...
pub async fn download_file(url: &str, dest: &Path, expected_sha256: Option<&str>) -> Result<()> {
    ...
    std::fs::rename(&temp_path, dest)
        .with_context(|| format!("Failed to move {} to {}", temp_path.display(), dest.display()))?;
